...
function edt_display_edit($year, $week, $filiere, $bdd) {
...
ini_set('display_errors', 1);
	...
	foreach ($daysToProceed as $day) { ?>
		...
		<tr>
			<th class="day"><?php echo $day; ?></th>
			<td class="checkbox" colspan=1> 
				<?php
				if (isset(${$day}[$numCoursDuJour]['dateedt'])) {?>
					<span class="copy" data-date="<?= ${$day}[$numCoursDuJour]['dateedt'] ?>" data-filiere="<?=$filiere?>">Copier</span><br/>
				<?php
				}
				?>
				<span class="paste" data-date="<?= ${$day}[$numCoursDuJour]['dateedt'] ?>">Coller</span>
			</td>
